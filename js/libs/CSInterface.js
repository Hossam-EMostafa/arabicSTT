...
*/
CSInterface.prototype.loadBinAsync = (urlName, callback) => {
    try {
    ...
    try {
        ...
        if (pathName.startsWith("file://")) {
            ...
            if (OSVersion.indexOf("Windows") >= 0) {
                localPath = pathName.replace("file:///", "");
            }
            else if (OSVersion.indexOf("Mac") >= 0) {
                localPath = pathName.replace("file://", "");
            }
            window.__adobe_cep__.loadSnapshot(localPath);
            return true;
        }
    }
    catch (err) {
 ...
 */
CSInterface.prototype.evalScript = (script, callback) => {
    const callbackFn = (callback === null || callback === undefined)
        ? (result) => { }
        : callback;
    window.__adobe_cep__.evalScript(script, callbackFn);
};
 ...
 */
CSInterface.prototype.getHostCapabilities = () => {
 ...
 */
CSInterface.prototype.dispatchEvent = (event) => {
 ...
 *         Default is null.
 */
CSInterface.prototype.addEventListener = (type, listener, obj) => {
 ...
 *          Default is null.
 */
CSInterface.prototype.removeEventListener = (type, listener, obj) => {
 ...
 *
 */
CSInterface.prototype.requestOpenExtension = (extensionId, params) => {
 ...
 */
CSInterface.prototype.getExtensions = (extensionIds) => {
 ...
 */
CSInterface.prototype.getNetworkPreferences = () => {
 ...
 */
CSInterface.prototype.initResourceBundle = () => {
 ...
 */
CSInterface.prototype.dumpInstallationInfo = () => window.__adobe_cep__.dumpInstallationInfo();
 ...
 */
CSInterface.prototype.getOSInformation = () => {
 ...
 *      </ul>\n
 */
CSInterface.prototype.openURLInDefaultBrowser = (url) => cep.util.openURLInDefaultBrowser(url);
 ...
 * @return extension ID.
 */
CSInterface.prototype.getExtensionID = () => window.__adobe_cep__.getExtensionId();
 ...
 *      </ul>\n
 */
CSInterface.prototype.getScaleFactor = () => window.__adobe_cep__.getScaleFactor();
...
if (navigator.appVersion.toLowerCase().indexOf("windows") >= 0) {
    CSInterface.prototype.getMonitorScaleFactor = () => window.__adobe_cep__.getMonitorScaleFactor();
}
 ...
 *
 */
CSInterface.prototype.setScaleFactorChangedHandler = (handler) => {
 ...
 *
 */
CSInterface.prototype.getCurrentApiVersion = () => {
 ...
 *
 */
CSInterface.prototype.setPanelFlyoutMenu = (menu) => {
 ...
 * </Menu>
 */
CSInterface.prototype.setContextMenu = (menu, callback) => {
    if ("string" !== typeof menu) {
        return;
    }
 ...
 *
 */
CSInterface.prototype.setContextMenuByJSON = (menu, callback) => {
    if ("string" !== typeof menu) {
        return;
    }
 ...
 * @param checked       True to select the item, false to deselect it.
 */
CSInterface.prototype.updateContextMenuItem = (menuItemID, enabled, checked) => {
 ...
 */
CSInterface.prototype.isWindowVisible = () => window.__adobe_cep__.invokeSync("isWindowVisible", "");
 ...
 */
CSInterface.prototype.resizeContent = (width, height) => {
 ...
 */
CSInterface.prototype.registerInvalidCertificateCallback = (callback) => window.__adobe_cep__.registerInvalidCertificateCallback(callback);
 ...
 *
 */
CSInterface.prototype.registerKeyEventsInterest = (keyEventsInterest) => window.__adobe_cep__.registerKeyEventsInterest(keyEventsInterest);
 ...
 */
CSInterface.prototype.setWindowTitle = (title) => {
 ...
 */
CSInterface.prototype.getWindowTitle = () => window.__adobe_cep__.invokeSync("getWindowTitle", "");